...
export class MicrosoftRewardsBot {
    ...
    private async runTasks(accounts: Account[]) {
        for (const account of accounts) {
            log('MAIN-WORKER', `Started tasks`,'log',account)
            ...
            log('MAIN-WORKER', `Completed tasks`,'log',account)
        }
    ...
    async Desktop(account: Account) {
        ...
        log('MAIN', 'Starting DESKTOP browser','log',account)
        ...
        log('MAIN-POINTS', `Current point count: ${data.userStatus.availablePoints}`,'log',account)

        const earnablePoints = await this.browser.func.getEarnablePoints()
        this.collectedPoints = earnablePoints
        log('MAIN-POINTS', `You can earn ${earnablePoints} points today`,'log',account)

        // If runOnZeroPoints is false and 0 points to earn, don't continue
        if (!this.config.runOnZeroPoints && this.collectedPoints === 0) {
            log('MAIN', 'No points to earn and "runOnZeroPoints" is set to "false", stopping!','log',account)
        ...
        if (this.config.workers.doDesktopSearch) {
            await this.activities.doSearch(workerPage, data,account)
        }
    ...
    async Mobile(account: Account) {
        ...
        log('MAIN', 'Starting MOBILE browser','log',account)
        ...
        if (!data.userStatus.counters.mobileSearch) {
            log('MAIN', 'No mobile searches found, stopping!','log',account)
        ...
        log('MAIN-POINTS', `The script collected ${this.collectedPoints} points today`,'log',account)
